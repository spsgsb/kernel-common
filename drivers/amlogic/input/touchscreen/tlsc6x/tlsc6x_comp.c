...
int tlsc6x_read_bytes_u16addr_nolock(struct i2c_client *client, u16 addr, u8 *rxbuf, u16 len)
{
    int err = 0;
    int retry = 0;
    u16 offset = 0;
    ...
    while(len > 0){
		...
		retry = 0;
		while (tlsc6x_i2c_read_nolock(client, buffer, 2, &rxbuf[offset], msgs[1].len) < 0) {
			if (retry++ == 3) {
				err = -1;
				break;
			}
...
int tlsc6x_read_bytes_u16addr(struct i2c_client *client, u16 addr, u8 *rxbuf, u16 len)
{
	...
	ret = tlsc6x_read_bytes_u16addr_nolock(client, addr, rxbuf, len);
...
/* in directly memory access mode */
/* RETURN:0->pass else->fail */
int tlsc6x_write_bytes_u16addr_nolock(struct i2c_client *client, u16 addr, u8 *txbuf, u16 len)
{
    ...
    while(len){
		...
		retry = 0;
		while (tlsc6x_i2c_write_nolock(client, buffer, msg.len) < 0) {
			if (retry++ == 3) {
				err = -1;
				break;
			}
...
int tlsc6x_write_bytes_u16addr(struct i2c_client *client, u16 addr, u8 *txbuf, u16 len)
{
	...
	mutex_lock(&i2c_rw_access);
	ret = tlsc6x_write_bytes_u16addr_nolock(client, addr, txbuf, len);
	mutex_unlock(&i2c_rw_access);
	return ret;
}
...
int tlsc6x_get_i2cmode(void)
{
    u8 regData[4];
    TLSC_FUNC_ENTER();
	if (tlsc6x_read_bytes_u16addr_nolock(g_tlsc6x_client, 0x01, regData, 3)) {
		return -EPERM;
	}
...
/* 0:successful */
int tlsc6x_set_dd_mode_nolock(void)
{
	int mod = -1;
	int retry = 0;
...
//         TLSC_FUNC_ENTER();
	...
	while (retry++ < 5) {
		msleep(20);
		tlsc6x_write_bytes_u16addr_nolock(g_tlsc6x_client, 0x42bd, cmd_2dma_42bd, 6);
		msleep(30);
...
int tlsc6x_set_dd_mode(void)
{
 ...
 //       TLSC_FUNC_ENTER();
	mutex_lock(&i2c_rw_access);
	ret = tlsc6x_set_dd_mode_nolock();
	mutex_unlock(&i2c_rw_access);
	return ret;
}

/* 0:successful */
int tlsc6x_set_nor_mode_nolock(void)
{
	int mod = -1;
	int retry = 0;
	u8 reg = 0x05;
  //      TLSC_FUNC_ENTER();
	while (retry++ < 5) {
		tlsc6x_write_bytes_u16addr_nolock(g_tlsc6x_client, 0x03, &reg, 1);
...
int tlsc6x_set_nor_mode(void)
{
   ...
   //     TLSC_FUNC_ENTER();
	mutex_lock(&i2c_rw_access);
	ret = tlsc6x_set_nor_mode_nolock();
	mutex_unlock(&i2c_rw_access);
	return ret;
}
...
{
	if (ptcfg == NULL) {
		TLSC_ERROR("is_valid_cfg_data fail \n");
		return 0;
	}
   TLSC_FUNC_ENTER();
   TLSC_INFO("ptcfg address:g_tlsc6x_client->addr:%d:%d\n",(ptcfg[53]>>1)&0x7f,g_tlsc6x_client->addr);
    if((u8)((ptcfg[53]>>1)&0x7f) != (u8)(g_tlsc6x_client->addr)){
    	TLSC_ERROR("ptcfg is fail \n");
        return 0;
    }

    if(tlsc6x_checksum_u16(ptcfg, 204)){
    	TLSC_ERROR("tlsc6x_checksum_u16 fail\n");
        return 0;
    }
...
static int tlsc6x_tpcfg_ver_comp(unsigned short * ptcfg)
{
	...
	if (g_tlsc6x_cfg_ver == 0) {	/* no available version information */
		TLSC_ERROR("g_tlsc6x_cfg_ver=:%d\n",g_tlsc6x_cfg_ver);
       	return 0;
	}

	if (is_valid_cfg_data(ptcfg) == 0) {
		TLSC_ERROR("fail is_valid_cfg_data\n");
		return 0;
	}
	...
	TLSC_INFO("ptcfg version error,g_tlsc6x_cfg_ver is 0x%x:ptcfg version is 0x%x!\n",
...
static int tlsc6x_tpcfg_ver_comp_weak(unsigned short * ptcfg)
{
	...
	if (is_valid_cfg_data(ptcfg) == 0) {
		TLSC_ERROR("cfg_data is invalid!\n");
		return 0;
	}
	...
	u32tmp = (u32tmp << 16) | ptcfg[0];
	if ((g_tlsc6x_cfg_ver & 0x3ffffff) != (u32tmp & 0x3ffffff)) {	/*  */
		TLSC_ERROR("ptcfg version error,g_tlsc6x_cfg_ver is 0x%x:ptcfg version is 0x%x!\n",
		g_tlsc6x_cfg_ver, u32tmp);
		return 0;
	}
...
int tlsx6x_update_fcomp_cfg(u16 *ptcfg)
{
	...
	if (tlsc6x_download_ramcode(fw_fcode_burn, sizeof(fw_fcode_burn))) {
		TLSC_ERROR("update error:ram-code error!\n");
		return -EPERM;
	}

	if (tlsc6x_write_burn_space((unsigned char *)ptcfg, 0x8000, 204)) {
		TLSC_ERROR("update fcomp_cfg fail!\n");
		return -EPERM;
	}

	TLSC_INFO("update fcomp_cfg pass!\n");
...
int tlsx6x_update_fcomp_boot(unsigned char* pdata, u16 len)
{
	...
	if (tlsc6x_write_burn_space((unsigned char *)pdata, 0x00, len)) {
		TLSC_ERROR("update fcomp_boot fail!\n");
		return -EPERM;
	}
	pdata[8] = 0x4b;
	if (tlsc6x_write_burn_space((unsigned char *)&pdata[8], 0x08, 1)) {
		TLSC_ERROR("update fcomp_boot last sig-byte fail!\n");
		return -EPERM;
	}

	TLSC_INFO("update fcomp_boot pass!\n");
...
int tlsx6x_update_ocomp_boot(unsigned char *pdata, u16 len)
{
	...
	if (tlsc6x_write_burn_space((unsigned char *)pdata, 0x00, len)) {
		TLSC_ERROR("update ocomp_boot fail!\n");
		return -EPERM;
	}
	...
	TLSC_INFO("update ocomp_boot pass!\n");
...
int tlsc6x_update_f_combboot(u8* pdata, u16 len)
{
	...
	TLSC_INFO("update file size is %d, and boot len is %d !\n",len,boot_len);

	if (g_mccode == 0) {
		if (g_needKeepRamCode == 0) {
			TLSC_ERROR("update_f_combboot error:mccode error!\n");
			return -EPERM;
		}
...
int tlsx6x_update_burn_cfg(u16* ptcfg)
{
	...
	if ((addr <= 0x6180) || (addr == 0x7f80)) {
		TLSC_ERROR("update error:time limit!\n");
		return -EPERM;
	}
	...
	if (tlsc6x_read_burn_space((unsigned char *)&check, addr - 256, 2)) {
		TLSC_ERROR("update error:pre-read error!\n");
		return -EPERM;
	}
	if (check != 0xffff) {
		TLSC_ERROR("update error:pre-read limit!\n");
		return -EPERM;
	}
...
int tlsc6x_load_gesture_binlib(void)
{
	...
	if (ret) {
		TLSC_ERROR("load gesture binlib error!\n");
		return -EPERM;
	}
	return 0;
}
...
static int tlsc6x_upgrade_romcfg_array(unsigned short *parray, unsigned int cfg_num)
{
	...
	if (g_tlsc6x_cfg_ver == 0) {	/* no available version information */
		TLSC_ERROR("no current version information!\n");
		return 0;
	}
	...
	if (tlsc6x_set_dd_mode()) {
		TLSC_ERROR("auto update error:can't control hw mode!\n");
		return -EPERM;
	}
...
int tlsc6x_update_compat_ctl(u8 *pupd, int len)
{
    ...
    TLSC_INFO("entry:tlsc6x_update_compat_ctl:len=%d\n",len);
	...
	if ((offset + upd_header->len_cfg + upd_header->len_boot) != len) {
		TLSC_ERROR("cfg len fail\n");
		return -EPERM;
	}
	
	if ((n * 204) != upd_header->len_cfg) {
		TLSC_ERROR("len_cfg fail\n");
		return -EPERM;
	}
...
int tlsc6x_tp_dect(struct i2c_client *client)
{
	...
	if (tlsc6x_get_running_cfg(tl_buf_tmpcfg) == 0) {
		...
		tlsc_cfg_version = g_tlsc6x_cfg_ver>>26;
		tlsc_vendor_id = (g_tlsc6x_cfg_ver>>9)&0x7F;
		TLSC_INFO("cfg_ver: 0x%x, chip_code: 0x%x, ver: %d, vendor_id: 0x%x\n",  
			g_tlsc6x_cfg_ver, g_tlsc6x_chip_code, tlsc_cfg_version, tlsc_vendor_id);
